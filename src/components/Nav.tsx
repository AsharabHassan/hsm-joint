    ...
    <nav className="sticky top-0 z-50 bg-cream/97 backdrop-blur-2xl border-b border-black/[0.04]">
      <div className="max-w-page mx-auto w-full px-4 md:px-8">
        ...
        <div className="flex items-center justify-between h-[62px]">
          <Link href={prefix || "/"} className="shrink-0 flex items-center gap-2.5 hover:opacity-90 transition-opacity">
            <Image
              src="/images/logo.png"
          ...
          <div className="hidden lg:flex items-center gap-1 ml-6">
            ...
            {navAreas.map((area) => {
              const isActive = pathname === `${prefix}/${area.slug}`;
              return (
                <Link
                  key={area.slug}
